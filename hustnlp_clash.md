# HUST NLP Server `clashctl` 使用教程

本文面向已经安装好 system 多用户版 `clashctl` 的服务器普通用户。每个人使用自己的订阅、节点、端口和 `mihomo` 进程；不需要 `sudo`，也不会影响其他用户。
...
新开的 Bash/Zsh 终端会自动加载。`clashctl` 是系统可执行命令，即使尚未加载 shell hook 也可以使用；`clashui`、`clashsub`、`clashnode` 等是兼容函数，必须先加载 shell hook。
